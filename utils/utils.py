...
def load_image(img_path: Union[str, Path], target_shape: Optional[Union[int, Tuple[int, int]]] = None) -> np.ndarray:
    """Load and optionally resize an image.
    
    Args:
        img_path: Path to the image file
        target_shape: Target shape for resizing. Can be:
            - None: no resizing
            - int: resize to this width, maintaining aspect ratio
            - tuple: resize to (height, width)
    
    Returns:
        Image as float32 numpy array in [0, 1] range
    """
    if not os.path.exists(img_path):
        raise FileNotFoundError(f'Path does not exist: {img_path}')
    img = cv.imread(str(img_path))[:, :, ::-1]  # [:, :, ::-1] converts BGR (opencv format...) into RGB
